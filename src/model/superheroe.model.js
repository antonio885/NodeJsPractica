...
Superheroes.update = (id,superheroe,result)=>{
    dbConn.query("UPDATE superheroe set nombre=?,apellido=?,nombre_heroe=?,ciudad=? WHERE id=?",[superheroe.nombre,
        superheroe.apellido,superheroe.nombre_heroe,superheroe.ciudad,id
    ],(err,res)=>{
        if(err){
            console.log("error", err);
            result(err,null)
